...
csrf <- GET(
  url='http://deq1.bse.vt.edu/d.dh/restws/session/token/',
...
for (i in 1:length(gage)) {   
	...
	P_est <- c()
	winterflow <- c()	
	gage_hydroid <- gagelist$HydroID...Unique.Feature.ID[i]
  ...
  print(paste("getting betas: ", beta_url, sep=""));
	...
	for (j in 1:length(month)) {
      ...
      if (length(b0) & length(b1)) {
        ...
        # varindex = Unique variable integer. will have to retrieve this from the dh_variabledefinition table
        # via REST
        varindex = 2; 
        x <- POST("http://deq1.bse.vt.edu/d.alpha/dh_timeseries/", 
                  add_headers(HTTP_X_CSRF_TOKEN = token),
                  body = list(
                    featureid = gage_hydroid,
                    entity_type = 'dh_feature',
                    varid = varindex,
                    tstime = as.numeric(as.POSIXct('2017-05-01', tz = "GMT")),
                    tsvalue = 1.5
                  ),
                  encode = "json",
                  verbose()
